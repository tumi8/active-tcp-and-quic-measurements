'''
    The script is extended from https://github.com/aiortc/aioquic/blob/main/examples/http3_client.py
'''
    ...
    for http_event in http_events:
        if isinstance(http_event, HeadersReceived):
            ...
            for k, v in http_event.headers:
                headers += k + b": " + v + b"\r\n"
                if "status".casefold() in k.decode("utf-8").casefold() and v.decode("utf-8").isnumeric():
                    meta_info['status'] = v.decode("utf-8")
                if "content-length".casefold() in k.decode("utf-8").casefold():
                    meta_info['content-length'] = v.decode("utf-8")
                if "server".casefold() in k.decode("utf-8").casefold():
                    meta_info['server'] = v.decode("utf-8")
        if isinstance(http_event, DataReceived):
            octets += len(http_event.data)
    meta_info['time'] = elapsed * 1000
    meta_info['octets'] = octets
    meta_info['download_speed'] = octets * 8 / elapsed / 1000000
    logger.info(
    ...
    if output_dir is not None:
        if headers:
            output_header = os.path.join(
                output_dir, "header.info"
            )
            with open(output_header, "w") as output_file:
                output_file.write(headers.decode("utf-8"))

        output_meta = os.path.join(
            output_dir, "meta_info.json"
        )
        with open(output_meta, "w") as output_file:
            json.dump(meta_info, output_file, indent=4)
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        filename=args.output_dir + "/quic_download.log",
    ...
    configuration = QuicConfiguration(
        is_client=True, idle_timeout=5, alpn_protocols=H0_ALPN if args.legacy_http else H3_ALPN
    )
    if args.ca_certs:
